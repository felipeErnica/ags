...
export const connectToAP = (accessPoint: AstalNetwork.AccessPoint, event: Astal.ClickEvent): void => {
    if (accessPoint.bssid === connecting.get() || isApActive(accessPoint) || event.button !== Gdk.BUTTON_PRIMARY) {
        return;
    }
    ...
    execAsync(`nmcli device wifi connect ${accessPoint.bssid}`)
        ...
        .catch((err) => {
            connecting.set('');
            if (err.message?.toLowerCase().includes('Senha é necessária, mas não foi fornecida')) {
                staging.set(accessPoint);
            } else {
                execAsync(`notify-send "Rede" "${err.message}"`)
                    .then()
